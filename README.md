### Step-by-Step Tutorial to Access Google API Services using Python (so original...)

#### Map Data
Access to accurate real-time map data has become crucial to our everday life, not just to businesses. But guess what, we can also access these databases, but how do we get access? Through API!
...
As with any other applications, proper usage of their APIs is well-documented, including their exact functionality as well as usage limitations and constraints. For more information, visit their official webiste [Google Places](https://cloud.google.com/maps-platform/places).

Here, we'll write a script that automatically makes requests to Google API and returns the results as a table. For implementation, we'll be coding everything using the Python programming language for its user-friendliness. In addition, we'll be utilizing the googlemaps python package, which is an easy-to-use Python wrapper for the Google Maps and Local Search API. 
...
```
The libraries time and googlemaps are used to artificially create delay in between code execution and access Google APIs services, respectively. In addition, we'll be needing geopandas and geojsonio in order to visualize our results as an interactive map.

#### Class Initialization
The approach we'll be taking is an object-oriented approach. This allows our code to be more organized, easy to modify, and reusable through inheritance. These advantages will be discussed in the later sections. 

First step is to create a class and attach two main attributes to it, namely, api_key and place_type. The former is used to contain the user API key needed for authenticating the use of Google APIs, while the latter contains the type of establishment we're interested in. 
...
Next, we define a method called `search_query()` that would contains the code that accept queries as input and returns the list of locations as output. To have access to Google services, we first need to authenticate our request by providing a unique API key as follows:  
```python
...
Now that we have access to Google APIs, we can start making request. Specifically, we need the latitude and longitude coordinates of the city that we're interested in. To accomplish this, we simply call the function `find_place()` and provide it with the three necessary arguments. The first argument should contain the query in the form of a string. To prevent the possibility of results being found outside the Philippines, we'll include the string `, Philippines` as part of our query. As requires an additional input from the user, we add the parameter `city` in the `search_query()` method. We'll then store the results in the `lat` and `lng` variables for used later.
```python 
    # define function to search query and return results
...
```python
      ...
      for j in range(2):
          ...
          try:
              ...
              break
```
Notice that I've included an exeption statement, as well as the line `time.sleep(3)` which sets a delay of three seconds for each iteration. This is in anticipation of errors arising from making more requests when there is no more results to be returned, or when the maximum limit of requests has been reached. 

Putting it all together, we now have created a class that can search for a cafes found within a specified city in the Philippines. 
```python 
class Gmaps_Search():
    # initialize class by defining api key and search query constaints
    def __init__(self, api_key, place_type):
...
Now that we're done writing our class, let's try to create an instance that searches of cafes in the city of Makati.
...
Another advantage is that we can create different instances of the same class. If our goal is search for banks instead of cafes, we simply replaced the type from `'cafe'` to `'bank'` when creating an instance. For a list of valid place types, visit the following link [Place Types](https://developers.google.com/places/supported_types).
